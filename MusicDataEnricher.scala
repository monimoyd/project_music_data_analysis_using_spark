...
class MusicDataEnricher( allDataFrameParam: DataFrame, broadcastStndIdGeoCdMapParam:Broadcast[HashMap[String, String]], broadcastSongArtistMapParam:Broadcast[Map[String, String]], broadcastUserArtistMapParam: Broadcast[Map[String, String]], broadcastUserSubscriptionParam: Broadcast[Map[String, (Long, Long)]]) extends Serializable {
	val allDataFrame: DataFrame =allDataFrameParam
 // val stndIdGeoCdMap:HashMap[String, String] = stndIdGeoCdMapParam
	val broadcastStndIdGeoCdMap:Broadcast[HashMap[String, String]] = broadcastStndIdGeoCdMapParam
	...
	def fillNullValueGeoCd = udf((stationId: String, geoCd: String) => {
	    val stndIdGeoCdMap1:HashMap[String, String] = HashMap("ST400" -> "A",
                          "ST401" -> "AU",
                          "ST402" -> "AP",
                          "ST403" -> "J",
                          "ST404" -> "E",
...
def findSubscribers = udf((userId: String, startTs: Long) => {
  ...
  if (subscriptionTuple._1 ==0 && subscriptionTuple._2 == 0 ) "0"
	else if (startTs >= subscriptionTuple._2  && startTs <=  subscriptionTuple._1) "1"
	else "0"
...
def enrichData():DataFrame = {
 ...
 newDataFrame = newDataFrame.withColumn("modified_Artist_id",  fillNullValueArtistId(newDataFrame("Song_id"), newDataFrame("Artist_id")))
 ...
 newDataFrame = newDataFrame.withColumn("isValid", validateRecords (newDataFrame("User_id"), newDataFrame("Song_id"), newDataFrame("modified_Artist_id"),  newDataFrame("modified_Geo_cd"), newDataFrame("Timestamp"), newDataFrame("Start_ts"), newDataFrame("End_ts")))
